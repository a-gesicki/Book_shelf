...
void
accessShelf (int option, vector<Book>& shelf, queue<Book>& dropbox,
             string filename, string dbfilename)
{
  if (option == 0)
  {
  ...
  else if (option == 4)
  {
    ...
    auto it = find_if (shelf.begin (), shelf.end (),
                       [item] (Book& b) { return b.getTitle () == item; });
    if (it != shelf.end ())
    {
...
void
printIntro (vector<Book>& shelf, queue<Book>& dropbox, string filename,
            string dbfilename)
{
  int option;
  cout << "\n"
